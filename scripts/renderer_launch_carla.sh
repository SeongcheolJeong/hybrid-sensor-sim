      ...
      while IFS='|' read -r record field1 field2 field3 field4; do
        case "${record}" in
          FRAME)
            ...
            ingest_frame_args+=(--ingest-frame "${renderer_frame_id}:${sensor_name}:${payload_artifact}")
            ;;
          META)
            sensor_name="${field1}"
            sensor_id="${field2}"
            ...
            ingest_meta_args+=(--ingest-meta "${sensor_name}:${sensor_id}:${data_format}:${attach_to}")
            ;;
        esac
      done <<<"${parsed_frames}"
      ;;
...
if backend and backend != "carla":
    raise SystemExit(f"ingestion profile backend mismatch: expected carla, got {backend}")
