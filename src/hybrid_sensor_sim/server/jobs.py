...
class JobManager:
    ...
    def _dispatch(self, run_type: str, payload: dict[str, Any], artifact_root: Path) -> dict[str, Any]:
        ...
        if run_type == "closed_loop_demo":
            return run_scenario_closed_loop_demo(
                scenario_path=str(self._payload_value(payload, "scenario_path", default="")),
                linux_runtime_root=str(self._payload_value(payload, "linux_runtime_root", default="")),
                autoware_workspace_root=str(self._payload_value(payload, "autoware_workspace_root", default="")),
                awsim_runtime_root=str(self._payload_value(payload, "awsim_runtime_root", default="")),
                map_path=str(self._payload_value(payload, "map_path", default="")),
                route_path=str(self._payload_value(payload, "route_path", default="")),
                out_root=artifact_root,
                runtime_backend_workflow_report_path=str(payload.get("runtime_backend_workflow_report_path", "")),
                backend_smoke_workflow_report_path=str(payload.get("backend_smoke_workflow_report_path", "")),
                autoware_pipeline_manifest_path=str(payload.get("autoware_pipeline_manifest_path", "")),
                autoware_dataset_manifest_path=str(payload.get("autoware_dataset_manifest_path", "")),
                autoware_topic_catalog_path=str(payload.get("autoware_topic_catalog_path", "")),
                autoware_consumer_input_manifest_path=str(payload.get("autoware_consumer_input_manifest_path", "")),
                run_duration_sec=float(payload.get("run_duration_sec", 45.0)),
                heartbeat_timeout_sec=float(payload.get("heartbeat_timeout_sec", 60.0)),
                poll_interval_sec=float(payload.get("poll_interval_sec", 2.0)),
                startup_grace_sec=float(payload.get("startup_grace_sec", 5.0)),
                record_video=bool(payload.get("record_video", True)),
                record_rviz=bool(payload.get("record_rviz", False)),
                record_rosbag=bool(payload.get("record_rosbag", False)),
                strict_capture=bool(payload.get("strict_capture", False)),
                preflight_only=bool(payload.get("preflight_only", False)),
                allow_non_linux_host=bool(payload.get("allow_non_linux_host", False)),
                awsim_launch_command=str(payload.get("awsim_launch_command", "")),
                autoware_launch_command=str(payload.get("autoware_launch_command", "")),
                route_goal_command=str(payload.get("route_goal_command", "")),
                localization_check_command=str(payload.get("localization_check_command", "")),
                perception_check_command=str(payload.get("perception_check_command", "")),
                planning_check_command=str(payload.get("planning_check_command", "")),
                control_check_command=str(payload.get("control_check_command", "")),
                vehicle_motion_check_command=str(payload.get("vehicle_motion_check_command", "")),
                route_completion_check_command=str(payload.get("route_completion_check_command", "")),
                video_capture_command=str(payload.get("video_capture_command", "")),
                rviz_capture_command=str(payload.get("rviz_capture_command", "")),
                rosbag_record_command=str(payload.get("rosbag_record_command", "")),
            )
