...
- `scripts/acquire_renderer_backend_package.py`: resolves an official AWSIM/CARLA package URL from `renderer_backend_local_setup.json`, prefers direct archive-style URLs over HTML release pages, reuses the setup summary `recommended_download_dir` when `--download-dir` is omitted, estimates archive size when possible, checks local free space in the selected download directory, downloads the archive, and optionally stages it into a runnable backend directory.
...
When local setup exposes a `recommended_download_dir`, both `renderer_backend_workflow --auto-acquire` and standalone `scripts/acquire_renderer_backend_package.py --setup-summary ...` now reuse it automatically if `--download-dir` is not provided. The selector already inspects mounted `/Volumes/*` disks plus any explicit `--download-dir-candidate` paths and `CARLA_PACKAGE_DOWNLOAD_DIRS` / `AWSIM_PACKAGE_DOWNLOAD_DIRS`, so packaged acquisition can pivot to the largest available external volume before the download starts.
