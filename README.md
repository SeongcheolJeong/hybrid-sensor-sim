...
- `scripts/acquire_renderer_backend_package.py`: resolves an official AWSIM/CARLA package URL from `renderer_backend_local_setup.json`, prefers direct archive-style URLs over HTML release pages, reuses the setup summary `recommended_download_dir` when `--download-dir` is omitted, reuses `recommended_stage_output_root` when `--output-root` is omitted, estimates archive size when possible, checks local free space in the selected download directory, downloads the archive, and optionally stages it into a runnable backend directory.
- `scripts/discover_renderer_backend_local_env.py`: now also surfaces per-backend `recommended_download_dir`, free-space readiness, and explicit `DOWNLOAD_SPACE_INSUFFICIENT` blockers so packaged runtime acquisition does not fail late. It also surfaces `recommended_stage_output_root`, extraction-space readiness, and explicit `STAGE_SPACE_INSUFFICIENT` blockers so staging can be redirected before archive expansion starts. It accepts extra `--download-dir-candidate` paths, honors `*_PACKAGE_DOWNLOAD_DIRS` / `BACKEND_PACKAGE_DOWNLOAD_DIRS` / `RENDERER_BACKEND_DOWNLOAD_DIRS` path lists, and automatically inspects mounted `/Volumes/*` candidates so larger external disks can be selected without manual restatement. Mounted volume candidates keep their `/Volumes/...` identity instead of collapsing through macOS symlink resolution, so recommended download and stage commands stay actionable.
...
- when `carla_local_v0` sees a packaged runtime blocker caused by insufficient free space, it now switches to the dedicated plan `packaged_runtime_required_with_download_space_blocker` and surfaces the recommended download directory in the same probe-set report
...
- when `carla_local_v0` sees archive extraction fail before staging because the output root is too small, it now switches to `packaged_runtime_required_with_stage_space_blocker` and surfaces the recommended stage/output directory plus its quantified shortfall in the same probe-set report
...
- when local setup already knows both a viable download directory and a viable stage/output root, the same probe-set/runtime-plan layer now prefers a combined acquire command that carries both `--download-dir` and `--output-root`, so package download and staging can be retried in one step instead of two separate commands
...
- the same probe-set report now also lifts `recommended_stage_output_root_counts`, `stage_output_root_status_counts`, and the primary stage/output-root shortfall fields into the top-level summary, so archive expansion blockers can be read without drilling into per-probe rows
...
When local setup exposes a `recommended_download_dir`, both `renderer_backend_workflow --auto-acquire` and standalone `scripts/acquire_renderer_backend_package.py --setup-summary ...` now reuse it automatically if `--download-dir` is not provided. The same setup summary also provides `recommended_stage_output_root`, and both flows now reuse it automatically when no explicit stage/output root is given. The selector already inspects mounted `/Volumes/*` disks plus any explicit `--download-dir-candidate` paths and `CARLA_PACKAGE_DOWNLOAD_DIRS` / `AWSIM_PACKAGE_DOWNLOAD_DIRS`, so packaged acquisition can pivot to the largest available external volume before the download starts and stage/archive expansion can land on the same high-capacity volume.
