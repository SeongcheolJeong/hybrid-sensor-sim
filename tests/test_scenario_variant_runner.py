...
class ScenarioVariantRunnerTests(unittest.TestCase):
    ...
    def test_run_scenario_variant_report_executes_rendered_scenario_definition_variants(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
                ...
                json.dumps(
                    {
                        ...
                        "variants": [
                            {
                                "scenario_id": "variant_direct_object_sim",
                                "logical_scenario_id": "logical_direct_object_sim",
            ...
            self.assertTrue(Path(report["variant_runs"][0]["summary_path"]).is_file())
            rendered_payload = json.loads(
                Path(report["variant_runs"][0]["rendered_payload_path"]).read_text(encoding="utf-8")
            )
