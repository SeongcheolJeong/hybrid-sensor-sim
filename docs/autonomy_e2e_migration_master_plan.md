...
- the real runtime probe-set layer now also aggregates `runtime_strategy` by probe plus blocker categories and recommended actions, so one pinned AWSIM/CARLA probe set can show readiness, recovery dependence, execution-route intent, and immediate troubleshooting focus together
