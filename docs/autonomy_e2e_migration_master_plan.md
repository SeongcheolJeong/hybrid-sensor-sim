...
- the real runtime probe-set layer now also aggregates `runtime_strategy` by probe plus strategy summary rows, blocker categories, recommended actions, and ordered resolution steps, so one pinned AWSIM/CARLA probe set can show readiness, recovery dependence, execution-route intent, and the immediate troubleshooting sequence together
...
- local setup now also accepts explicit extra download-volume candidates, inspects mounted `/Volumes/*` roots automatically, preserves those mounted path prefixes without symlink collapse, and honors `*_PACKAGE_DOWNLOAD_DIRS` environment path lists, so runtime strategy can pivot to an external or larger volume before packaged acquire/stage runs
...
- the same local setup/probe-set path now also promotes `recommended_stage_output_root`, `STAGE_SPACE_INSUFFICIENT`, and quantified extraction shortfall bytes, so archive expansion blockers are separated from download blockers instead of being collapsed into generic packaged-runtime failure
...
- when local setup already has both a usable download directory and a usable stage/output root, the same runtime-plan layer now prefers a combined acquire command carrying both `--download-dir` and `--output-root`, reducing packaged-runtime recovery to one retry step
- the same probe-set summary now also lifts the recommended download directory, download-directory status counts, and shortfall bytes to top-level fields, so CARLA package blockers can be triaged without opening individual probe rows
- the same probe-set summary now also lifts the recommended stage/output root, stage-output-root status counts, and shortfall bytes to top-level fields, so extraction-space blockers can be triaged without opening individual probe rows
...
2. real packaged backend smoke execution against AWSIM/CARLA binaries
   ...
   - the primary AWSIM visible-camera contract now also supports logical companion outputs, so the same primary camera payload can materialize `camera_semantic_json` as an additional output instead of requiring a separate camera sensor surface for semantic consumers
   - the Autoware bridge now treats embedded semantic payload inside the primary visible-camera export as a logical `camera_semantic` output, so fresh real AWSIM primary artifacts can satisfy `semantic_perception_v0` without a supplemental pass
   ...
   - the repository now also has a Linux-host-oriented `scenario_closed_loop_demo_workflow` orchestration path for `AWSIM + Autoware ROS2` demo runs, including preflight contract checks, helper-command launch orchestration, and video/rosbag artifact capture; the live runtime assets remain external to this repo
...
6. a clean `READY` packaged AWSIM/CARLA smoke run on native Linux x86_64 is still pending; the current macOS Docker handoff path now yields a real usable-success state for complete backend-runtime outputs instead of only a generic failure
...
The current runtime/backend bridge already exports Autoware ingest-oriented manifests with profile-based processing-stage readiness and concrete processing-stage bundle manifests, so downstream integration work should build on that surface rather than adding parallel topic-summary schemas.
