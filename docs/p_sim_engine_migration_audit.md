...
43. staged-backend selection reuse in scenario smoke workflows
   ...
   - local setup now also has a dedicated `CARLA` Docker pull probe, so the current machine can distinguish `image missing` from `docker pull blocked by Docker Desktop/containerd storage failure`; right now the concrete blocker is `meta.db` I/O failure during pull, not an unresolved code path in the repo
