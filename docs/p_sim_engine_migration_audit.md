...
47. Autoware data-contract bridge
   ...
   - `src/hybrid_sensor_sim/tools/scenario_runtime_backend_probe.py` now wraps that rebridge path into a compact pass/fail probe surface, which makes repeatable real AWSIM `tracking READY` and `semantic recovery READY` checks easier to pin as named probes
