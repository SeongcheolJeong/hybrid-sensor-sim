...
43. staged-backend selection reuse in scenario smoke workflows
   ...
   - local setup now also has a dedicated `CARLA` Docker pull probe and a broader Docker storage probe, so the current machine can distinguish `image missing` from `docker pull blocked by Docker Desktop/containerd storage failure`; right now the concrete blocker is image-store I/O failure, not an unresolved code path in the repo
