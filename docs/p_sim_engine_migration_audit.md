...
47. Autoware data-contract bridge
   ...
   - that supplemental semantic path now has an explicit strategy surface too: `auto`, `off`, and `dual_pass`, so semantic consumers can force a second semantic pass instead of waiting for a missing-topic trigger
