...
43. staged-backend selection reuse in scenario smoke workflows
   ...
   - local setup now also classifies Docker storage failures, so the current machine can distinguish `image missing` from `image_store_corrupt`; right now the concrete blocker is Docker image-store corruption, not an unresolved code path in the repo
