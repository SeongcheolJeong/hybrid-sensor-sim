...
47. Autoware data-contract bridge
   ...
   - the rebridge report now also preserves a `source vs refreshed` comparison section, so `ATTENTION -> SUCCEEDED`, `DEGRADED -> READY`, merged supplemental-report count changes, semantic recovery source, and missing-required-topic delta are visible without diffing two separate runtime reports
