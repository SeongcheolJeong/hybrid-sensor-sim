...
47. Autoware data-contract bridge
   ...
   - the rebridge report now also preserves a `source vs refreshed` comparison section, so `ATTENTION -> SUCCEEDED`, `DEGRADED -> READY`, or merged supplemental-report count changes are visible without diffing two separate runtime reports
