...
43. staged-backend selection reuse in scenario smoke workflows
   ...
   - `scenario_runtime_backend_probe_set.py` now also aggregates runtime-strategy counts and reason codes plus blocker categories and recommended actions, so the pinned real AWSIM probe set can report execution-route intent, downstream readiness, and immediate troubleshooting focus in one compact surface
