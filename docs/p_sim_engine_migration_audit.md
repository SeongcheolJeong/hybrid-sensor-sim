...
- the same local setup path now accepts explicit extra download-volume candidates and `*_PACKAGE_DOWNLOAD_DIRS` path-list env vars, so package acquisition can be redirected to a larger external volume without changing the downstream runtime workflows
