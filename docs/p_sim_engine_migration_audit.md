...
47. Autoware data-contract bridge
   ...
   - `src/hybrid_sensor_sim/tools/scenario_runtime_backend_probe_set.py` pins the canonical real AWSIM `tracking READY` and `semantic recovery READY` checks as a named probe set and directly separates `runtime_native_ready_probe_ids` from `supplemental_dependency_probe_ids`
