...
47. Autoware data-contract bridge
   ...
   - the bridge now also materializes `autoware/topics/...` export directories plus `autoware_topic_export_index.json`, and both smoke workflows lift `topic_export_count` / `materialized_topic_export_count` so downstream topic availability is visible at top level
