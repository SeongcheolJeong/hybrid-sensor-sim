...
- `scenario_runtime_backend_probe_set.py` now also includes a `carla_local_v0` path that consumes the latest `renderer_backend_local_setup.json` directly, so CARLA runtime blockers can be summarized in the same probe-set surface even before a real packaged-runtime workflow report exists
- `scenario_runtime_backend_probe_set.py` also now exposes `hybrid_runtime_readiness_v0`, which combines the pinned real AWSIM probe paths with the current CARLA local-runtime blocker probe so both backends can be triaged together
- the CARLA acquire path now prefers archive-style download URLs and uses the archive filename for the local target, which removes the previous dead-end where the GitHub release page itself was treated as a downloadable package
- the same acquire path now also estimates remote archive size and compares it to free space in the chosen download directory, so `CARLA` package blockers can be reported as explicit disk-capacity failures rather than generic download errors
- `renderer_backend_local_setup.py` now lifts that disk-capacity diagnosis into runtime strategy output (`recommended_download_dir`, `download_directory_status`, `DOWNLOAD_SPACE_INSUFFICIENT`), and `scenario_runtime_backend_probe_set.py` turns it into a dedicated packaged-runtime recovery plan for `carla_local_v0`
- the same local setup path now accepts explicit extra download-volume candidates, auto-inspects mounted `/Volumes/*` roots, preserves those mounted path prefixes without symlink collapse, and honors `*_PACKAGE_DOWNLOAD_DIRS` path-list env vars, so package acquisition can be redirected to a larger external volume without changing the downstream runtime workflows
...
- the same local setup/probe-set surface now also emits `recommended_stage_output_root`, `stage_output_root_status`, and quantified extraction shortfall bytes, so CARLA archive expansion blockers show up as `STAGE_SPACE_INSUFFICIENT` instead of hiding behind generic packaged-runtime failure
   ...
   - when local setup already has both a usable download directory and a usable stage/output root, the same runtime-plan layer now prefers a combined acquire command carrying both `--download-dir` and `--output-root`, so packaged-runtime recovery can be retried in one step
   ...
   - the same probe-set summary now also lifts the recommended stage/output root itself plus stage-status counts and extraction shortfall bytes into top-level fields, so stage-capacity blockers can be read directly from the summary header
...
47. Autoware data-contract bridge
   ...
   - the bridge now also carries run-level lineage into `autoware_pipeline_manifest.json` and `autoware_dataset_manifest.json`, including `variant_id`, `logical_scenario_id`, `scenario_id`, `source_payload_kind`, `smoke_scenario_path`, `bridge_manifest_path`, `available_modalities`, and `data_roots`
   ...
   - the bridge now also materializes `autoware/topics/...` export directories plus `autoware_topic_export_index.json`, `autoware_topic_catalog.json`, and `autoware_consumer_input_manifest.json`, and both smoke workflows lift `topic_export_count` / `materialized_topic_export_count` so downstream topic availability is visible at top level
   - `autoware_consumer_input_manifest.json` now also exposes `subscription_specs`, `sensor_inputs`, and `static_transforms`, so downstream consumers can bind topic subscriptions and frame transforms without re-parsing the flat topic catalog
   - the same bridge now emits `autoware_topic_catalog.json`, lifting `required_topic_count`, `missing_required_topic_count`, and `available_message_types` into both the bridge report and top-level scenario workflows
   - the bridge now also supports downstream consumer profiles; `semantic_perception_v0` and `tracking_fusion_v0` raise required-topic expectations so runtime-origin outputs can be classified as `DEGRADED` even when backend smoke/output comparison still report `COMPLETE` + `MATCHED`
   - the current AWSIM Linux-handoff runtime path now reproduces a real runtime-origin `DEGRADED` case under `semantic_perception_v0`: backend output comparison remains `MATCHED`, smoke remains `COMPLETE`, output origin remains `BACKEND_RUNTIME_ONLY`, but the top-level workflow degrades because the semantic camera topic is still absent
   - the same AWSIM Linux-handoff path also reproduces a runtime-origin `READY` case for `tracking_fusion_v0` when radar tracks are enabled, so both sides of the downstream consumer-profile contract are now covered by real runtime artifacts
   - for `semantic_perception_v0`, the backend smoke workflow can now trigger a supplemental semantic-only smoke run and merge that second report into the Autoware bridge, which gives the runtime path a concrete way to recover the missing semantic topic instead of treating it as a permanently degraded contract
   - that supplemental semantic path now has an explicit strategy surface too: `auto`, `off`, and `dual_pass`; for `semantic_perception_v0` on `AWSIM`, `auto` now promotes to an effective `dual_pass`, so the packaged runtime path can recover semantic output without an extra manual override
   - the primary AWSIM visible-camera path now also supports logical companion camera outputs, so `camera_projection_json` can carry additional `camera_semantic_json` materialization through the same primary sensor contract instead of forcing a second camera definition
   - that live supplemental semantic pass now uses its own isolated `renderer_backend_workflow` root too, so recovery cannot accidentally reuse the primary run's nested smoke artifacts
   - `scenario_runtime_backend_workflow` now also lifts that merged-report state to top-level `status_summary` and artifact paths, so supplemental semantic recovery can be triaged without opening the nested backend smoke workflow report
   - `src/hybrid_sensor_sim/tools/scenario_runtime_backend_rebridge.py` can now rebuild the top-level runtime report and Autoware bundle from an existing `scenario_runtime_backend_workflow_report_v0.json` or `scenario_backend_smoke_workflow_report_v0.json`, which shortens iteration time when only bridge/status logic changed after a real run was already captured
   - `src/hybrid_sensor_sim/tools/scenario_runtime_backend_probe.py` now wraps that rebridge path into a compact pass/fail probe surface
   - `src/hybrid_sensor_sim/tools/scenario_runtime_backend_probe_set.py` now pins three canonical real AWSIM checks as a named probe set: `tracking_ready`, `semantic_primary_ready`, and `semantic_recovery_ready`; this makes it explicit that fresh primary runtime artifacts can already satisfy `semantic_perception_v0` without supplemental recovery, while the older degraded artifact remains as a recovery regression guard
   - the rebridge report now also preserves a `source vs refreshed` comparison section, so `ATTENTION -> SUCCEEDED`, `DEGRADED -> READY`, merged supplemental-report count changes, semantic recovery source, missing-required-topic delta, and recovered-topic lists are visible without diffing two separate runtime reports
   - when a `semantic_perception_v0` runtime artifact is still missing the semantic topic and no supplemental report path was recorded, rebridge can now reconstruct a semantic-only supplemental smoke run from the source backend smoke report and merge it into the refreshed Autoware bundle automatically
   - that semantic supplemental rerun now uses an isolated `renderer_backend_workflow` root, which allows a real degraded AWSIM handoff artifact to be recovered to `READY` instead of accidentally reusing the source run's nested smoke artifacts
   - `src/hybrid_sensor_sim/tools/scenario_closed_loop_demo_workflow.py` now adds Linux-host-only `AWSIM + Autoware ROS2` closed-loop orchestration, including preflight contract validation from the current JSON Autoware bridge, helper-script launch coordination, and video/rosbag artifact capture; the live ROS2 runtime assets still remain outside this repo
